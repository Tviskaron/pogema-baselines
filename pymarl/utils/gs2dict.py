...
def generate_variants(unresolved_spec: Dict,
    ...
    There are two types of unresolved values:
        ...
        into the `random` or `np` module).
            "cpu": lambda spec: spec.configs.num_workers
            "batch_size": lambda spec: random.uniform(1, 1000)
    Finally, to support defining specs in plain JSON / YAML, grid search
    and lambda functions can also be defined alternatively as follows:
        "activation": {"grid_search": ["relu", "tanh"]}
        "cpu": {"eval": "spec.configs.num_workers"}
...
def count_variants(spec: Dict, presets: Optional[List[Dict]] = None) -> int:
    ...
    for preset in presets:
        ...
        deep_update(preset_spec["configs"], preset)
...
def get_preset_variants(spec: Dict,
                        config: Dict,
                        constant_grid_search: bool = False):
    """Get variants according to a spec, initialized with a configs.
    Variables from the spec are overwritten by the variables in the configs.
    ...
    for path, val in resolved:
        try:
            domain = _get_value(spec["configs"], path)
            ...
            raise ValueError(
                f"Pre-set configs key `{'/'.join(path)}` does not correspond "
                f"to a valid key in the search space definition. Please add "
                f"this path to the `configs` variable passed to `tune.run()`."
            ) from exc
        ...
        assign_value(spec["configs"], path, val)
